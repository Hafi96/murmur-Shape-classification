...
# Get shape status from patient data.
def get_shape(data):
    shape = next((line.split(': ')[1].strip() for line in data.split('\n') if line.startswith('#Systolic murmur shape:')), None)
    if shape is None:
        raise ValueError('No shape available. Is your code trying to load labels from the hidden data?')
    return shape
...
def save_challenge_outputs(filename, patient_id, classes, labels, probabilities):
    ...
    expected_classes = ["Diamond","Plateau","Decrescendo"]
