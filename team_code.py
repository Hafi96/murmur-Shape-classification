...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    shape_classifier = Hierachical_MS_Net(num_classes=DATASET_CFG['num_systolic_murmur_class'], **MODEL_CFG).to(device)
    optimizer = torch.optim.AdamW(shape_classifier.parameters(), **OPTIMIZER_CFG)
    ...
    # Stage 1: Train the classifier for shape classification
    if verbose:
        print('Training model for Systolic murmur shape classification...')
    for epoch in range(TRAINING_CFG['epochs']):
        if verbose:
            print(f'Epoch {epoch} starts...')
        train_epoch(train_loader, shape_classifier, optimizer, criterion, scheduler, device, TRAINING_CFG['print_freq'])
        if verbose:
            print('\n')
...
@torch.no_grad()
def recording_shape_diagnose(multi_scale_specs, shape_classifier, systolic_murmur_class, interval):
    shape_logits = shape_classifier(multi_scale_specs)
    shape_probs = F.softmax(shape_logits, -1).cpu().numpy()
    location_preds = calc_pred_locations(shape_probs, 
...
def run_challenge_model(model, data, recordings, verbose):
    ...
    recording_shape_preds = np.zeros(len(recordings), dtype=np.int_)

    for i in range(len(recordings)):
        multi_scale_specs, shapes = preprocessor(recordings[i], 4000, interval=interval)
        multi_scale_specs = [s.to(device) for s in multi_scale_specs]
        recording_shape_preds[i] = recording_shape_diagnose(multi_scale_specs, shape_classifier, systolic_murmur_class, interval)

    recording_shape_counts = np.bincount(recording_shape_preds, minlength=len(systolic_murmur_class))

    # Assign murmur labels based on the most frequent prediction
    shape_labels = np.zeros(len(systolic_murmur_class), dtype=np.int_)
    shape_labels[np.argmax(recording_shape_counts)] = 1
    classes=systolic_murmur_class
    probabilities = recording_shape_counts / np.sum(recording_shape_counts) if np.sum(recording_shape_counts) > 0 else np.zeros(len(systolic_murmur_class))

    return classes, shape_labels, probabilities
...
def save_challenge_model(model_folder, model, file_name='shape_classifier'):
    torch.save({
...
def load_challenge_model(model_folder, verbose):
    ...
    shape_checkpoint = torch.load(os.path.join(model_folder, 'shape_classifier.pth'), map_location=device)
    shape_classifier = Hierachical_MS_Net(num_classes=DATASET_CFG['num_systolic_murmur_class'], **MODEL_CFG).to(device)
    shape_classifier.load_state_dict(shape_checkpoint['model_state_dict'])
    ...
    return (device, preprocessor, shape_classifier, systolic_murmur_class)
