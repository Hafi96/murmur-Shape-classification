...
def convert_label_to_int(str_label):
    return shape_mapping_str2int[str_label]

def convert_label_to_str(int_label):
    return shape_mapping_int2str[int_label]

def get_shape_locations(data):
    locations = None
    for l in data.split('\n'):
...
def load_recordings_with_labels(data_folder, included_labels=["Diamond", "Plateau", "Decrescendo"]):   
    patient_files_arr, recording_files, shapes = [], [], []
    patient_files = find_patient_files(data_folder)
    
    for pf in patient_files:
        patient_data = load_patient_data(pf)
        patient_shape = get_shape(patient_data)

        # Skip patients whose murmur shape is not in the included labels
        if patient_shape not in included_labels:
            continue
        
        patient_shape_idx = included_labels.index(patient_shape)  # Assign index-based labels
        locations = get_locations(patient_data)
        shape_locations = get_shape_locations(patient_data)
        p_recordings = get_patient_recording_files(patient_data, len(locations))

        # Assign labels based on location rules
        for i in range(len(locations)):
            if included_labels[patient_shape_idx] == "Diamond" and locations[i] not in shape_locations:
                continue
            elif included_labels[patient_shape_idx] == "Plateau" and locations[i] not in shape_locations:
                continue
            elif included_labels[patient_shape_idx] == "Decrescendo" and locations[i] not in shape_locations:
                continue
            else:
                patient_files_arr.append(pf)
                recording_files.append(os.path.join(data_folder, p_recordings[i]))
                shapes.append(patient_shape_idx)
    ...
    shapes = np.array(shapes, dtype=np.int_)

    return patient_files_arr, recording_files, shapes
